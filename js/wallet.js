...
var rciqrcode = new QRCode(document.getElementById("rciqr"), {
    width: 100,
    height: 100,
    position: "center"
})
...
window.onload = function (){
    ...
    if (apiget == "mainnet" || apiget == null) {
        api = "http://20.243.210.93:4321"
        prefix = "RCI"
        inputPlaceholder.attr("placeholder", "Raffahcoin Address")
        ...
        api = "http://20.243.210.93:4321"
        prefix = "tRCI"
        inputPlaceholder.attr("placeholder", "tRaffahcoin Address")
        href = "http://20.243.210.93:4321/address/" + getaddress
        $("#history").attr("href", href)
    }
    ...
    function apiCall() {
        return Promise.resolve($.ajax({
            url: api + "/api/address/" + $("#addressInput").val(),
            dataType: 'json',
            type: 'GET'
        }))
    }
    ...
    function getBalance() {
        ...
        apiCall().then(function(data) {
            var getbalance = (data.balance).toFixed(8);
    ...
    function getReceived() {
        $("#currentReceived").text("loading...")
        apiCall().then(function (data) {
            var getreceived = (data.valueIn).toFixed(8);
            var received = getreceived
            $("#currentReceived").text(received + " " + prefix)
            })
    }
    ...
    function calcSpent() {
        ...
        apiCall().then(function (data) {
            var getreceived = (data.valueIn).toFixed(8);
            var getbalance = (data.balance).toFixed(8);
            var spent = (getreceived - getbalance).toFixed(8);
    ...
    function checkAPI() {
        apiCall().then(function(data) {
            if (data.error == null) {
                ...
                $("#currentBalance").text("Enter a valid RCI address")
                $("#currentReceived").text("")
                $("#currentSpent").text("")
            }
        })
    }
...
/*
  const url = "http://20.243.210.93:4321/api/address/" + getaddress;

  fetch(url)
    .then(response => response.json())
    .then(data => {
      const balance = data.balance.toFixed(2);
      const valueIn = data.valueIn.toFixed(2);

      // Menampilkan hasil balance dan valueIn
      console.log("Balance:", balance);
      console.log("ValueIn:", valueIn);

      // Memperbarui elemen HTML dengan ID yang sesuai
      document.getElementById("currentBalance").textContent = balance + " RCI";
      document.getElementById("currentReceived").textContent = valueIn + " RCI";
      document.getElementById("currentSpent").textContent = (valueIn - balance).toFixed(8) + " RCI";

      document.getElementById("currentBalance").insertAdjacentHTML('afterend', '<br>');
      document.getElementById("currentReceived").insertAdjacentHTML('afterend', '<br>');
    })
    .catch(error => {
...
function qrcodegen() {
    if (!document.getElementById("addressInput").value) {
        rciqrcode.makeCode("Enter an address")
    }
    else {
        rciqrcode.makeCode(document.getElementById("addressInput").value)
    }
}
